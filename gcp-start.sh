#!/bin/bash
#
# Start script handler for GCP minecraft-docker image
#
...
gsutil -m cp -r gs://$GCS_BUCKET$GCS_BUCKET_PATH/config /config/

chmod +x /start.sh
/start.sh

# Copy files again once the server is done
gsutil -m cp -r /config gs://$GCS_BUCKET$GCS_BUCKET_PATH/